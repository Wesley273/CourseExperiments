...
public class InToPost {
    ...
    public static void main(String[] args) {
        ...
        int result = count(toInversePolish(a));
    ...
    //中缀转后缀
    public static char[] toInversePolish(char[] ch) {
        String str = "";
        for (int i = 0; i < ch.length; i++) {
            ...
            if (ch[i] >= '0' && ch[i] <= '9') {
                ...
                if (isEmpty2() || ch[i] == '(' || operatorPriority(ch[i]) > operatorPriority(peek2())) {
                    push2(ch[i]);
                } else if (ch[i] == ')') {
                    while (peek2() != '(') {
                        push(pop2());
                    ...
                    while (!isEmpty2() && operatorPriority(ch[i]) <= operatorPriority(peek2()) && peek2() != '(') {
                        push(pop2());
                    }
                    push2(ch[i]);
